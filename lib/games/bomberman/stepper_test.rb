...
class Games::Bomberman::StepperTest < Minitest::Test
  ...
  def assert_score(movesets, initial, expected)
    players = movesets.each_with_index.map { |moveset, i| SetPlayer.new(i, moveset, Games::Bomberman) }

    state = Games::Bomberman::State.parse(StringIO.new(initial))
    actions = players.map { |player| player.next(state) }

    state = Games::Bomberman::Stepper.next(state, actions)

    assert_equal(state.score, expected)
  end
...
w w   w w
  ...
  def test_capture_of_coin_increases_score
    assert_score([%w[left]], <<-INITIAL, { 0 => 1 })
1
0 0
w w  w   w
w c1 p0o w
w w  w   w
  ...
  def test_simultaneous_capture_of_coin_increases_both_scores
    assert_score([%w[right], %w[left]], <<-INITIAL, { 0 => 1, 1 => 1 })
1
0 0 1 0
w w   w  w   w
w p0o c1 p1o w
